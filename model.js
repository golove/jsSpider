...
function findImgsrc(html) {
    let b = /https:\/\/p99.ajpiczs.com\/([^<>"\']*)\.jpg/gi
    let srcs = html.match(b)
    // console.log(srcs)
    if (srcs === null) return
    // 匹配 img 的 title 值
    let t = /\<span id="subject_tpc"\>.*\<\/span\>/gi
    let titlelist = html.match(t)
    // console.log(titlelist)
    return { 'title': titlelist[0], 'href': srcs, 'star': 0, 'collect': false, 'delete': false, 'download': false }
};


module.exports = {
    getPage, findHref, findImgsrc
